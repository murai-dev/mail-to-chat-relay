...
条件に合うメールを検知し、Discord などのチャットサービスへ通知するツールです。

重要なメールを担当者が手動で確認していると、確認漏れや対応遅れが起きやすくなります。このツールは、件名などの条件に合うメールを定期的にチェックし、必要な情報をチャットへ自動通知することで、対応の初動を早めることを目的としています。  

## 想定する用途

- 重要な問い合わせメールをチャットに通知したい
- 予約、申込、障害通知などを見逃しにくくしたい
- メール確認をチームの共有フローに乗せたい
- 小さな通知連携を低コストで作りたい
...
メールとチャットを連携する、業務通知自動化のサンプルです。
実際の業務では、Slack、Google Chat、Teams、LINE などへの通知や、通知内容の整形にも応用できます。
